...
ANIMATION_TIME = 1200 # tijd in ms voor beweging kaart

class Display:
    def __init__(self, windowName):
        glutInit()
        glutInitDisplayMode(GLUT_MULTISAMPLE | GLUT_DOUBLE) # double buffering voor animatie
    ...
    animationCallbacks = [] # lijst voor callback functions
    
    def animate(self): # animate function
    ...
    def endDraw(self):
        glFlush()
        glutSwapBuffers() # wissel buffers voor animatie
...
class Card:
    ...
    def draw(self, display):
        display.drawCard(self.posX, self.posY, self.colour, self.value, 1)
...
class Pile:
    ...
    def animate(self): # animation function voor bewegende kaart
    ...
    def moveCard(self, c):
        self.vectorX = float(c.posX - self.posX) / ANIMATION_TIME # bereken bewegingsvector
    ...
    def draw(self, display):
        if self.size() > 0:
            display.drawCard(self.posX, self.posY, None, "", self.size())
            self.topCard().draw(display)
        else:
            display.drawCard(self.posX, self.posY, WHITE, "", 0)
        if self.movingCard != None: # extra call voor bewegende kaart
            self.movingCard.draw(display)
...
class Player:
    ...
    def tryCard(self, pile, piles):
        if pile.topCard().value == 1:
            ...
            return True
        for i in piles:
            if i.movingCard == None and not i.isEmpty():
...
class Game:
    def __init__(self, noOfPlayers, msec):
        ...
        glutIdleFunc(self.display.animate) # idle function voor animatie
        glutMainLoop()
