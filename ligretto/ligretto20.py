...
class Display:
    def __init__(self, windowName):
        ...
        glOrtho(0, WIDTH, 0, HEIGHT, 0, 40) 
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glEnable(GL_BLEND)
        glEnable(GL_LINE_SMOOTH)
        glEnable(GL_DEPTH_TEST)

    def reshape(self, x, y): # reshape function
        if x / y > WIDTH / HEIGHT: # bereken viewport
...
class Game:
    def __init__(self, noOfPlayers, msec):
        ...
        glutReshapeFunc(self.display.reshape) # reshape function
        glutMainLoop()
    ...
    def mouseFunc(self, button, state, x, y):
        if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
            dummy, dummy, width, height = glGetInteger(GL_VIEWPORT) # Viewport kan anders zijn dan window
            x = WIDTH * x / width
